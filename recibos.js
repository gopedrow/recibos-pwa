...
function carregarDados() {
  ...
  fetch('https://script.google.com/macros/s/AKfycbxXdWDONd_EA6LSl3KNb0u6g7pI5vOsGUidmEzIwkHBX3WJbDLkYBqslqtNSbKsKNY/exec?tipo=cliente')
    .then(res => res.json())
    .then(clientes => {
      const filtroSelect = document.getElementById('filtroClienteSelect');
...
function renderizarClientesRecibos(clienteFiltro = '') {
  ...
  if (recibosParaMostrar.length === 0) {
    tbody.innerHTML = '<tr><td colspan="4" style="text-align: center; padding: 20px;">Nenhum recibo encontrado.</td></tr>';
    return;
  }
  ...
  if (!clienteFiltro) {
    Object.keys(recibosPorCliente).forEach(cliente => {
      ...
      trCliente.innerHTML = `<td colspan="4" style="background:#f0f0f0;color:#444;font-weight:700;border-radius:12px;">${cliente}</td>`;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('filtroClienteSelect').addEventListener('change', function(e) {
